...
export default function FinQuest(){
  ...
  useSecretAdmin(useCallback(()=>setScreen("admin-login"),[]) );
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange(async (event,session)=>{
      if(event==="SIGNED_OUT"){ setUser(null);setCash(100000);setXp(0);setPortfolio({});setTrades([]);setProgress({});setScreen("landing"); }
    });
  ...
  function sellStock(ticker,qty){
    ...
    setCash(newCash);
    setPortfolio(p=>{
      if(nq===0){const np={...p};delete np[ticker];return np;}
      return{...p,[ticker]:{...p[ticker],qty:nq}};
    });
  ...
  const sharedProps={user,courses,events,stocks,portfolio,cash,trades,xp,xpPct,level,LVL_NAMES,
    totalWealth,pnl,progress,lastUpdated,mktLoading,apiStatus,fetchPrices,
  ...
  if(screen==="landing")     return <Landing onLogin={()=>setScreen("login")} onSignup={()=>setScreen("signup")} stocks={stocks}/>;
  ...
  if(screen==="admin-login") return <AdminLogin onSuccess={()=>setScreen("admin")} onBack={()=>setScreen("landing")}/>;
  if(screen==="admin")       return <AdminPanel courses={courses} setCourses={setCourses} events={events} setEvents={setEvents} onExit={()=>setScreen("landing")}/>;
  ...
  return(
    <>
      ...
      <AppShell {...sharedProps} onLogout={async()=>{await supabase.auth.signOut();setScreen("landing");}}/>
    </>
  );
}
...
function AppShell(props){
  ...
  const nav=[
    {icon:"📊",label:"Dashboard",   k:"dashboard"},
    {icon:"🎓",label:"Academy",     k:"academy",  s:"Aprender"},
    {icon:"🧮",label:"Calculadoras",k:"calc"},
    {icon:"📈",label:"Simulador",   k:"sim",      s:"Investir"},
    {icon:"📰",label:"Radar",       k:"events"},
    {icon:"⚔️",label:"Duelo",       k:"duel",     s:"Competir",badge:"NOVO"},
    {icon:"🏆",label:"Ranking",     k:"ranking"},
    {icon:"🤖",label:"IA Tutora",   k:"ai",       s:"Exclusivo",badge:"NOVO"},
    {icon:"👤",label:"Perfil",      k:"profile",  s:"Conta"},
  ];
  return(
    <>
      <div className="sidebar">
        <div className="s-logo"><div className="logomark">FQ</div><span className="syne-text">FinQuest</span></div>
        <nav className="s-nav">
          {nav.map(item=>(
            <div key={item.k}>
              {item.s&&<div className="s-section">{item.s}</div>}
              <div className={"nav-item"+(page===item.k?" act":"")} onClick={()=>setPage(item.k)} title={item.label}>
...
function DashPage({cash,totalWealth,pnl,portfolio,stocks,xp,xpPct,level,LVL_NAMES,courses,progress,trades,lastUpdated,mktLoading,apiStatus,fetchPrices}){
  const totalPort=totalWealth-cash;
  // Generate benchmark data relative to 100k baseline
      ...
      <div className="g2" style={{marginBottom:18}}>
        <div className="card">
          ...
          <div style={{display:"flex",gap:10,marginTop:12}}>
            {[["Carteira",myReturn,"var(--g)"],["CDI",cdiReturn,"var(--gold)"],["Ibovespa",ibovReturn,"var(--blue)"]].map(([l,v,c])=>(
...
function CalcPage(){
  ...
  return(
    <div className="page">
      <div className="topbar"><div><div className="ptitle syne">Calculadoras</div><div className="psub">Simule seus investimentos</div></div></div>
...
function ApoCalc(){
  ...
  const anos=s.a-s.i;const fv=s.r*300;const taxa=0.008;const m=anos*12;
  ...
  return(
    <div className="g2">
      ...
      <div style={{background:"var(--bg1)",border:"1px solid var(--b)",borderRadius:12,padding:28,display:"flex",flexDirection:"column",justifyContent:"center"}}>
        ...
        <div style={{marginTop:12,color:"var(--muted)",fontSize:13,lineHeight:1.6}}>Com {anos} anos e 0,8%/mês, precisará acumular <span style={{color:"var(--g)",fontWeight:700}}>{fmt(fv)}</span> para gerar a renda desejada.</div>
      </div>
    </div>
  );
}
...
// ─── RANKING ──────────────────────────────────────────────────────
function RankPage({totalWealth,user,cash}){
  ...
  useEffect(()=>{
    supabase.from("profiles").select("id,name,xp,cash").order("cash",{ascending:false}).limit(50)
  ...
  return(
    <div className="page">
      <div className="topbar"><div><div className="ptitle syne">Ranking Global</div><div className="psub">Maiores patrimônios do simulador</div></div></div>
      ...
      <div className="card" style={{marginBottom:18,borderColor:"rgba(0,214,143,.2)",background:"rgba(0,214,143,.04)",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px"}}>
        <div style={{display:"flex",alignItems:"center",gap:11}}>
          <div className="syne" style={{fontSize:24,fontWeight:800,color:"var(--g)",width:40,textAlign:"center"}}>
            {myPos?`#${myPos}`:"—"}
          </div>
          <div className="avatar" style={{width:36,height:36,fontSize:12}}>{user?.name?.slice(0,2).toUpperCase()}</div>
          <div><div style={{fontWeight:700}}>Você — {user?.name}</div><div style={{fontSize:11,color:"var(--muted)"}}>Patrimônio simulado</div></div>
        </div>
        ...
        {list.map((r,i)=>{
          ...
          return(
            <div key={r.id} style={{display:"flex",alignItems:"center",gap:11,padding:"13px 18px",borderBottom:i<list.length-1?"1px solid var(--b)":"none",background:isMe?"rgba(0,214,143,.05)":""}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontWeight:600}}>{r.name}{isMe&&<span style={{fontSize:10,color:"var(--g)",marginLeft:6}}>← você</span>}</div>
                <div style={{fontSize:11,color:"var(--muted)"}}>{lvlName(r.xp)} · {r.xp} XP</div>
              </div>
              <div className="syne" style={{fontWeight:800,fontSize:17}}>{fmt(r.cash)}</div>
            </div>
          );
        })}
      </div>
  ...
  async function send(){
    ...
    try{
      const payload={
        ...
        max_tokens:1000,
        system:`Você é a IA Tutora do FinQuest, plataforma brasileira de educação financeira gamificada.
Ensine sobre investimentos de forma clara e didática para brasileiros. Foque em: renda fixa, ações, FIIs, ETFs, Tesouro Direto, CDI, SELIC, IPCA, B3, dividendos, carteiras.
Use exemplos em reais (R$). Formate com **negrito** para termos importantes. Seja conciso e encorajador.
Nunca dê recomendações específicas de investimento. Sempre diga que o usuário deve estudar antes de investir dinheiro real.`,
  ...
  return(
    <div className="page">
      <div className="topbar">
        <div>
          <div className="ptitle syne">🤖 IA Tutora</div>
          <div className="psub">Tire dúvidas sobre investimentos com inteligência artificial</div>
        </div>
